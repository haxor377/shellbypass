...
function logout()
{
    setcookie('authenticated', '', time() - 3600, "/"); // Unset the cookie
...
if (!isset($_COOKIE['authenticated'])) {
    ...
    $stored_hashed_password = '$2y$10$.ooaan3I5nZA/r5hzgqv9.7iU2FkyFtD8BxYcg0HNSNdsArr0oJ8.'; // Use PASSWORD_DEFAULT hash u can use this site https://phppasswordhash.com/ for generate ur password

    if (isset($_POST['username']) && isset($_POST['pass'])) {
        if ($_POST['username'] === $stored_username && password_verify($_POST['pass'], $stored_hashed_password)) {
            setcookie('authenticated', true, time() + (86400 * 30), "/"); // Cookie valid for 30 days
            header("Location: " . $_SERVER['PHP_SELF']);
            exit;
        } else {
            show_login_page("Invalid username or password. Please try again.");
        }
    } else {
        show_login_page();
    }
